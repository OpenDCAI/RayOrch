...
from rayorch import DagExecutor, DagPipeline, Dispatch, RayModule
...
class DemoPipe(DagPipeline):
    ...
    def forward(self, x: list[int]) -> tuple[list[int], list[int]]:
...
if __name__ == "__main__":
    ...
    out = DagExecutor(pipe, max_batches_inflight=4).run(xs)
    print(out)
