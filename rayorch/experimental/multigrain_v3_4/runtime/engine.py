"""Multigrain v3.4 的单写者、事件驱动 Arena 语义状态机。"""
...
class ArenaEngine:
    ...
    def admit_sources(
        ...
        *,
        ...
        demanded = set(self.program.source_ports).intersection(
            self.program.control_ports
        )
    ...
    def commit_success(self, report: CallReport) -> None:
        ...
        for output_port in expected_outputs:
            ...
            scalar_control_required = (
                not expanded_ports and output_port in self.program.control_ports
            )
            ...
            for child_port in expanded_ports:
                ...
                control_required = child_port in self.program.control_ports
        ...
        if self.entity_count + new_entities > self.limits.max_entities:
    ...
    def _try_filter(self, target_port: PortRef, entity: EntityRef) -> None:
        ...
        if mask_record.control:
            control = None
            if target_port in self.program.control_ports:
