...
class AssembleNested:
    ...
    def run(self, documents, groups):
        """保留完整嵌套形状，输出用于 唯一 Executor parity。"""
...
def run_batching_matrix(
    *,
    parents: int = 1_000,
    batch_sizes: tuple[int, ...] = (16, 32, 64),
    rounds: int = 8,
) -> list[DummyMetrics]:
    """运行 2×3 Ray batching 消融，并对每个 batch cap 强制输出 parity。"""
...
def run_inflight_matrix(
    *,
    ...
    for in_flight in in_flights:
        pipeline = _dummy_pipeline(batch_size=batch_size, rounds=rounds)
        with Executor(pipeline, address="auto") as executor:
            result = executor.run(
                range(parents),
...
def _dummy_pipeline(*, batch_size: int, rounds: int):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mode", choices=("batching", "inflight", "nested"), default="batching")
    parser.add_argument("--parents", type=int, default=None)
    parser.add_argument("--rounds", type=int, default=8)
    args = parser.parse_args(argv)

    if args.mode == "batching":
        records = run_batching_matrix(
            parents=args.parents or 1_000,
            rounds=args.rounds,
        )
    elif args.mode == "inflight":
        records = run_inflight_matrix(
            parents=args.parents or 256,
            rounds=args.rounds,
        )
    else:
        parents = args.parents or 128
        with Executor(NestedDummyPipeline(rounds=args.rounds)) as executor:
            result = executor.run(range(parents))
        records = [
            {
                "mode": "nested",
                "parents": parents,
