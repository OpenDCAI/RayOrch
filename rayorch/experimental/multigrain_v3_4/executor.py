"""基于 Ray actor pools 的 v3.4 执行器与多 Arena driver。
...
class _RayWorkerActor:
    ...
    def __init__(
        ...
        self._worker = Worker(target, init_args, init_kwargs)
...
class Executor:
        ...
        *source_columns: Iterable[Any],
        ...
        while len(completed) < len(slices):
            ...
            if not pending:
                ...
                raise RuntimeError(f"v3.4 Ray runtime deadlocked: {summaries}")
    ...
    def _admit_arena(
        ...
        for port, values in zip(self.program.source_ports, columns):
            ...
            if port in self.program.control_ports:
