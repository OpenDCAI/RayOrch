"""使用 v3.3 Port API 回归真实 Flash-MinerU PDF→Page→OCR→Document。

业务 UDF 与 v3 runner 完全复用，避免模型、render 或 assemble 逻辑漂移；本模块只替换
Pipeline 表达和执行器。4/48/368 三个 gate 因而能直接比较 v3 与 v3.3 的语义输出及
...
class MinerUV33Pipeline(Pipeline):
    """显式建模 Expand/Reduce 的真实 MinerU v3.3 Pipeline。"""
...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    """运行一次 v3.3 MinerU gate，并增量写入摘要与 GPU samples。"""
...
def build_parser() -> argparse.ArgumentParser:
    """构造 v3.3 MinerU 4/48/368 通用 CLI。"""
