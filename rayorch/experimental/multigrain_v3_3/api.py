"""Multigrain v3.3 的 RayModule 与符号化 Pipeline 编写 API。"""
...
_ACTIVE_TRACE: contextvars.ContextVar[_ProgramBuilder | None] = (
    contextvars.ContextVar("multigrain_v3_3_trace", default=None)
)
    ...
    def build(self, output_tree: object) -> CompiledProgram:
        ...
        for call, outputs in program.outputs_by_call.items():
            ...
            for output in outputs:
                ...
                is_mask = any(
                    isinstance(spec.origin, FilterOrigin)
                    and spec.origin.mask_port == output
                    for spec in program.ports.values()
                )
                layouts.append(OutputLayout(output, expanded, is_mask))
