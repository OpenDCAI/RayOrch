"""Paired V3↔V3.6 video correctness and performance gate.
...
def _first_difference(left: list[Any], right: list[Any]) -> str:
    if len(left) != len(right):
        return f"output length differs: V3={len(left)}, V3.6={len(right)}"
...
def _run_trial(
    ...
    *,
    ...
    correctness = _compare_outputs(outputs["v3"], outputs["v36"])
    ...
    trial = {
        ...
        "v36_rpc_count": rpc_counts["v36"],
        "v3_grains_per_rpc": grains_per_rpc["v3"],
        "v36_grains_per_rpc": grains_per_rpc["v36"],
        "v3_output_digest": _digest(outputs["v3"]),
        "v36_output_digest": _digest(outputs["v36"]),
...
def run_paired(args: argparse.Namespace) -> dict[str, Any]:
    ...
    v36_walls = [float(trial["v36_wall_s"]) for trial in trials]
    payload = {
        "schema_version": 1,
        ...
        "v36_wall_s": [round(value, 6) for value in v36_walls],
        "v3_median_wall_s": round(statistics.median(v3_walls), 6),
        "v36_median_wall_s": round(statistics.median(v36_walls), 6),
        "v36_speedup_median": round(
            statistics.median(
                old / new for old, new in zip(v3_walls, v36_walls)
            ),
            6,
        ),
        ...
        "v36_output_digest": trials[-1]["v36_output_digest"],
        "trials": trials,
    }
    if args.output:
