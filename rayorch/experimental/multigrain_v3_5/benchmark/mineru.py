"""使用 v3.5 compiler/runtime 回归真实 Flash-MinerU 368-PDF workload。
...
class MinerUV35Pipeline(Pipeline):
...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    """运行一次 v3.5 MinerU gate，并写入摘要和 GPU samples。"""
    ...
    if result is None:  # pragma: no cover - exception path exits in the try block
        raise RuntimeError("v3.5 MinerU run produced no result")
...
def build_parser() -> argparse.ArgumentParser:
    """构造 v3.5 MinerU 4/48/368 通用 CLI。"""
...
__all__ = ["MinerUV35Pipeline", "build_parser", "run_benchmark"]
