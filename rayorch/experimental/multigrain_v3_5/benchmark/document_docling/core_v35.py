"""Docling Document→Page→TableJob→Page→Document on Multigrain v3.5.
...
``RayModule + F.*``, the v3.5 compiler, RuntimePlan, Arena and Executor.
"""
...
class DoclingTableJobV35Pipeline(Pipeline):
...
class DoclingTableFormerV1BatchV35Pipeline(DoclingTableJobV35Pipeline):
    """The promoted mutation-free TableFormer V1 batch kernel on v3.5."""
...
class DoclingTableFormerV2BatchV35Pipeline(DoclingTableJobV35Pipeline):
    """The opt-in TableFormer V2 batch kernel on v3.5."""
...
def run_v35(
    ...
    **pipeline_options: Any,
    ...
    if mode == "v1_batch":
        pipeline_type = DoclingTableFormerV1BatchV35Pipeline
    elif mode == "v2_batch":
        pipeline_type = DoclingTableFormerV2BatchV35Pipeline
    else:
        pipeline_type = DoclingTableJobV35Pipeline
