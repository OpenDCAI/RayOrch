"""Paired V3↔V3.5 Docling 4/48/368 correctness/performance gate."""
...
def _arm_options(
    ...
    """Return V3 options and the explicit v3.5-supported projection."""
...
def _worker_summary(result: RunResult) -> dict[str, Any]:
    """Aggregate observation-only v3.5 Worker snapshots by UDF class."""
...
def _run_trial(
    ...
    *,
    ...
    if jaccard and min(jaccard) < minimum_jaccard:
        raise ValueError(
            f"V3↔V3.5 minimum Markdown Jaccard {min(jaccard)} "
            f"is below {minimum_jaccard}"
        )
    audit_violations = tuple(
        f"{engine}: {violation}"
        for engine in ("v3", "v35")
...
def run_paired(args: argparse.Namespace) -> dict[str, Any]:
    ...
    v35_measured = [trial["arms"]["v35"]["measured_s"] for trial in trials]
    v3_outer = [trial["arms"]["v3"]["outer_wall_s"] for trial in trials]
    v35_outer = [trial["arms"]["v35"]["outer_wall_s"] for trial in trials]
    payload = {
        "schema_version": 1,
        ...
        "summary": {
            ...
            "v35_median_outer_wall_s": statistics.median(v35_outer),
            "v35_speedup_outer_wall_median": statistics.median(
                old / new for old, new in zip(v3_outer, v35_outer)
            ),
            "engine_measured_scope_note": (
                "V3 measured excludes RunResult.get(); V3.5 measured includes "
                "materialization, so outer wall is the primary paired metric"
            ),
            "v3_median_measured_s": statistics.median(v3_measured),
            "v35_median_measured_s": statistics.median(v35_measured),
            "v35_engine_measured_ratio_median": statistics.median(
