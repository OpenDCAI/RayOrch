...
attempt, ExecutionMicrobatch, queue, or generation; callers provide immutable facts
...
class RecoveryPolicy:
    ...
    ``max_udf_retries`` excludes the initial execution. For ``ISOLATE_TAIL``
    it covers the whole-batch retry before isolation, not subsequent splits.
    """

    udf_mode: UdfRecoveryMode = UdfRecoveryMode.ABORT
    max_udf_retries: int = 0
    infra_retries: int = 1
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.max_udf_retries) is not int:
            raise TypeError("max_udf_retries must be an integer")
        if self.infra_retries < 0:
            raise ValueError("infra_retries must be non-negative")
        if self.udf_mode is UdfRecoveryMode.ABORT:
            if self.max_udf_retries != 0:
                raise ValueError("abort recovery does not accept UDF retries")
        elif self.udf_mode in {
            UdfRecoveryMode.RETRY_BATCH,
            UdfRecoveryMode.RETRY_TAIL,
        }:
            if self.max_udf_retries <= 0:
                raise ValueError("retry recovery requires positive max_udf_retries")
        elif self.udf_mode is UdfRecoveryMode.ISOLATE_TAIL:
            if self.max_udf_retries != 1:
                raise ValueError(
                    "isolate_tail performs exactly one whole-ExecutionMicrobatch "
                    "deferred retry"
                )
    ...
    def retry_batch(
        ...
        *,
        ...
        """Retry through the immediate queue, up to ``max_retries`` extra times."""

        return cls(UdfRecoveryMode.RETRY_BATCH, max_retries, infra_retries)
    ...
    def retry_tail(
        ...
        *,
        ...
        """Retry through the deferred queue, up to ``max_retries`` extra times."""

        return cls(UdfRecoveryMode.RETRY_TAIL, max_retries, infra_retries)

    @classmethod
    def isolate_tail(cls, *, infra_retries: int = 1) -> "RecoveryPolicy":
        """Defer one retry, then split a still-failing ExecutionMicrobatch to singletons."""
    ...
    def decide_udf(
        ...
        *,
        ...
        if type(grain_count) is not int or grain_count <= 0:
            raise ValueError("UDF recovery requires a non-empty ExecutionMicrobatch")
        match self.udf_mode:
            case UdfRecoveryMode.ABORT:
                return RecoveryAction.ABORT
            case UdfRecoveryMode.RETRY_BATCH | UdfRecoveryMode.RETRY_TAIL as mode:
                if completed_retries >= self.max_udf_retries:
                    return RecoveryAction.ABORT
                return (
    ...
    def allows_infrastructure_retry(
        ...
        """Return whether every Grain in one ExecutionMicrobatch retains retry budget."""

        if not completed_retries:
            raise ValueError(
                "infrastructure recovery requires a non-empty ExecutionMicrobatch"
            )
