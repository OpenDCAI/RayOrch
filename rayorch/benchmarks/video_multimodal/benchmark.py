"""Public Benchmark for the video-multimodal reference workload."""
...
from .pipeline import VideoMultimodalTopologyPipeline


@dataclass(frozen=True, slots=True)
class VideoMultimodalTopologyBench:
    output_dir: str | Path = "."
    video_count: int = 2
    frames_per_video: int = 4
    audio_chunks_per_video: int = 3
    workers: int = 2
    batch_size: int = 4
        ...
        *,
        ...
        videos = tuple(
            (
                ...
                self.audio_chunks_per_video,
            )
            for index in range(self.video_count)
        )
        return run_benchmark(
            name="video_multimodal_topology",
            pipeline=VideoMultimodalTopologyPipeline(
    ...
    def submit(
        ...
        *,
        ...
        return submit_benchmark(
            benchmark="video_multimodal_topology",
            config=benchmark_config(self),
            artifact_root=Path(self.output_dir) / ".rayorch-benchmark",
            target=target,
            source=source,
...
def _metrics(result: RunResult) -> dict[str, int]:
    ...
    return {
        "audio_chunks": sum(len(output["audio"]) for output in outputs),
        "frames": sum(len(output["vision"]) for output in outputs),
    }


__all__ = ["VideoMultimodalTopologyBench"]
