...
| `ItemOutcome` / `RecordFailure` / `GroupFailure` | 表达精确坏项或同父隔离结果 |
| `RecoveryPolicy` | 只在需要异常重试或二分隔离时配置 |
...
`ray_options()` 中的 `batching_policy` 只控制 READY Grain 怎样组成物理
`DispatchBatch`：`"any_parent"` 是默认值，允许同一 Call 下不同直接父级共同填满一批；
`"single_parent"` 要求一批只包含同一个 `parent_anchor`，局部性更强但可能形成较小 batch。
它不改变 Domain/Entity lineage、`GroupFailure` 作用域或下游语义。旧的预发布
`batch_scope="elastic"|"parent_bound"` 写法会被明确拒绝，不保留 compatibility alias。
...
`RecordFailure` / `GroupFailure`，Engine 再预检完整 batch reports；同一 Grain 的 M 个输出
要么一起生效，要么一个都不生效。因此每个输出 Item 有独立身份，但它们共享同一次 Grain
commit 边界。
...
- `SUPPRESSED`：上游失败或同 Call、同直接父级的 suppression barrier 使当前计算不应再运行/提交。
...
| `READY + SUPPRESS -> SEALED` | 出队时命中已建立的同父 suppression barrier | 否 |
...
得到全部 24 种 `(phase, event)` 组合。表中六种必须返回指定的新 phase，其余十八种必须
...
| Executor → Worker | `GrainInvocation`、`CallInputLayout`、`CallOutputLayout` | `WorkerReport` / `DispatchFailure` | Worker 只改 actor 内 UDF 状态 |
...
写入 canonical table并进入 `_fact_queue` 队列；Executor、Worker 和 materializer 都没有第二个
publication 入口。
...
flowchart TD
    ...
    Group --> PerParent["当前 Grain FAILED；同 Call+直接父的未提交兄弟 SUPPRESSED"]
    Udf --> Policy["RecoveryPolicy：abort / retry / isolate"]
    Infra --> Replace["按策略替换 actor 并 replay"]
    Contract --> Fast["确定性错误 fail-fast，不重试"]
```

两种显式数据判断都是普通返回值，并要求 UDF 返回完整、行对齐的 batch：

```python
return mg.RecordFailure("bad page")       # 只失败当前 Grain
return mg.GroupFailure("bad document")   # 再隔离同 Call、同直接父级的未提交兄弟
```

它们都不触发 retry。`GroupFailure` 只建立 microbatch-local barrier：已经提交的结果不回滚，
已发给其他 actor 的计算不追杀，但其晚到成功结果会在 driver commit 时变成 `SUPPRESSED`；其他
parent 和其他 Call 正常提交。可恢复的不透明错误仍应 `raise`，交给 `RecoveryPolicy`。
