# RayOrch Multi-Grain v3.3：Port / Domain 驱动的运行时

> 状态：**权威设计规范（implementation contract）**。
>
> v3.3 是独立新内核，不通过 lowering 复用 v3/v3.2 executor。v3 仅作为 table-driven runtime、lineage、GroupShape、atomic commit、recovery 与实验行为的参考。
...
v3.3 第一版规定 Domain graph 是 rooted forest：每个非根 Domain 恰有一个 parent。
...
BlockTable: BlockRef -> BlockRecord
```

ValueTable 与 ItemTable 分离。ObjectRef 丢失、block compact 或 replay 不改写 Item terminal semantics。
...
不存在 ItemRecord 表示未 terminal，不保存 PENDING。PRESENT Item 必须在同一 commit 中获得合法 ValueBinding。
...
```text
ScalarEmit
ExpandEmit(child domains/reporters)
MaskEmit
IgnoreEmit
```

`ExpandEmit` 是 compiler/runtime 根据 Port demand 生成的物理计划，不进入 CallSpec，RayModule 不知道下游存在 Expand。
...
- lost BlockRef 通过 Item→Port provenance→producer Grain 找到最小 replay 集；
...
```text
multigrain_v3_3/
├── model.py        # refs、outcomes、records；无 Ray
├── program.py      # immutable Program/Specs；无 runtime/Ray
├── api.py          # Pipeline/RayModule symbolic tracing
├── functional.py   # Port structural API
├── compile.py      # normalize/validate/index/ExecutionPlan
├── protocol.py     # stable DTO；无 runtime/Ray
├── runtime/
│   ├── state.py    # passive tables/progress records
│   ├── group.py    # pure hierarchical GroupAccumulator
│   └── engine.py   # sole Arena state machine
├── driver.py       # multi-Arena coordination
├── execution.py    # Ray pools/ObjectRefs/pending RPC
├── worker.py       # UDF ABI/layout execution
├── executor.py     # public facade/assembly
└── benchmark/
```
...
compile        -> program/model
runtime        -> program/model/protocol
driver         -> runtime/execution/protocol
execution      -> model/protocol
worker         -> protocol
executor       -> compile/driver/execution
```

禁止：

```text
model/program -> runtime/driver/execution/Ray
runtime       -> Ray/actor handles
execution     -> Arena internals/PortOrigin
worker        -> Program/Arena/Driver
metrics       -> correctness decisions
executor      -> duplicate semantic state machine
```

不使用 mixin、callback 修改 Arena、全局 mutable registry、actor-to-actor RPC 或跨层私有字段访问。
...
对比保持：v3.3 elastic、v3.3 parent-bound、现有 v3、Ray Data、Native。相同
...
现有文档中 Page→Region 和 deterministic bad leaf 主要是计划，不是已完成真实实验。它们必须先成为 v3.3 conformance/integration tests，再升级为真实 workload。
...
Phase 3  Worker/local executor + dummy benchmark
Phase 4  Ray single-Arena integration
...
### 24.1 验收结果（更新于 2026-08-04）

- v3.3 全量回归：`44 passed`；
- local/Ray dummy 覆盖 parent-bound/elastic、multi-Arena、nested、bad leaf 和 retry；
- 4 PDF 与 48 PDF gate 均完成，文档集合完整且逐文档 Jaccard 全部 ≥0.98；
- 368 PDF / 7,072 pages 的 clean elastic 已完成；旧 parent-bound 已完成
  correctness/packing，但 clean paired wall-time 尚待重跑；
- 两种模式均为 368 docs、7,072 pages、13 个 Call actors、Arena HWM 3；
- clean elastic 为 `592.975s / 11.9263 pages/s`，相对历史 v3
  `587.781s / 12.0317 pages/s` 只差约 0.88%，处于真实模型约 1% 的正常波动范围；
- 2026-08-03 的 elastic `635.947s / 11.1204 pages/s` 与另一套占满 4 GPU 的
  Ray session 重叠，已标记为 invalid，不再支持“v3.3 吞吐回退 7.6%”的结论；
- clean elastic OCR 为 121 RPC / 58.45 grains per RPC，旧 parent-bound 为
  368 RPC / 19.22；该证据仍证明 `batch_scope` 只改变 packing；
- clean elastic 对历史 v3 为 368/368 文档匹配，Jaccard median 0.9962；
- 完整数值、污染诊断、correctness 和 artifact 路径见
  `docs/experiments/multigrain_v3/v3_3_regression_matrix.md`。

计时口径由 actor-ready 屏障显式定义：`startup_s` 包含 UDF/模型构造，
`measured_wall_s` 只包含 `run()`。参考执行器采用即时、work-conserving 聚批，不暴露尚未
实现的定时等待参数。性能 gate 还必须记录 GPU/Ray preflight；资源隔离失败时，
`measured_wall_s` 不具备可比性。
