...
| [`编译边界`](multigrain_v3_6_compiler_boundary.md) | compiler 维护者 | 为什么需要固定编译流水线 | 不讲 Ray event loop |
...
| ValueTable | ItemRef | RowBinding 或 NestedGroupBinding |
...
| 状态 | 数据结构 | 含义 |
| --- | --- | --- |
| Grain records | `dict[GrainRef, GrainRecord]` | phase、generation、infra failures、冻结的 parent anchor |
| READY work | `dict[CallRef, deque[GrainRef]]` | 按 Call 分区的首次 READY Grain |
| immediate-retry work | `deque[DispatchBatch]` | 优先重试 exact DispatchBatch |
| deferred-recovery work | `deque[DispatchBatch]` | 延后重试或二分后的 DispatchBatch |

同父数据隔离只额外使用 Engine 内私有的 `_SuppressionBarrierIndex`，底层键为
`(CallRef, parent_anchor)`。它不拥有 Grain phase/queue，也不进入 Worker；三个队列和
late report 都用 `GrainRecord.parent_anchor` 查询这一张 microbatch-local barrier 表。
...
```text
_fact_queue             = 已成立、待传播的语义 Ref FIFO
pending_grains     = 尚未决定的 Call input slots dict
ready/immediate-retry/
deferred-recovery  = READY Grain 物理选择 queues
...
Entity、Item、Expansion 首次 publication 会把其 Ref 加入 `_fact_queue`。Grain phase 不进入语义 FIFO，
...
| Reduce | child→parent 一层 | Expansion + members + values | 建 NestedGroupBinding，不执行聚合 UDF |
...
- `GrainFailureReport`：确定到一个 Grain 的显式失败，并用内部二值位区分是否建立同父 barrier；
...
一个完整 WorkerDispatchResult 分四阶段：

1. reports 与 DispatchBatch grains 一一对应，且 phase/generation 全部有效；
2. 按 `DispatchBatch.grains` 顺序预扫描所有 `GroupFailure` barrier；
3. 只为最终 live 的成功报告验证 output、expanded rows、control 和 aligned cardinality，并构造
   immutable commit intents；
4. 显式失败 `FAILED`、barrier 命中成功 `SUPPRESSED`、其余成功正常 publication，最后只调用一次
   `advance()`。
...
| `GroupFailure` | UDF 返回某行 `GroupFailure` | `GrainFailureReport(suppress_siblings=True)` | 当前 Grain FAILED；同 Call+直接父的未提交兄弟 SUPPRESSED |
...
Engine          -> batch commit、suppression barrier 与最终 Item/Expansion publication
...
两种显式 Failure 都是完整 batch 中的正常返回值，不经过 `RecoveryPolicy`。suppression barrier 建立后，
READY 在出队门禁封闭、WAITING 在输入 admission 封闭、晚到成功在 commit 门禁封闭；已经提交的
PRESENT 不回滚，已经发给 actor 的 RPC 不取消。opaque/infra failure 对应的 `DispatchBatch` 先按 barrier
分区，只有 live subset 消耗 retry budget。
...
- `_fact_queue` 和 pending Call slots 为空；
- Dispatch runnable/recovery queues 为空；
- 所有 Grain SEALED；
- public output Port × Entity 全部终态；
- Executor 没有该 microbatch 的 pending RPC。
...
| stale report | Grain generation 与 DispatchBatch | Dispatch walkthrough |
...
6. [`executor.py：actor slots、pending RPCs 与 event loop`](multigrain_v3_6_walkthrough/06_executor_event_loop.md)
