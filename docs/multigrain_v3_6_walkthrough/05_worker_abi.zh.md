...
编译好的 input/output layout 和一批物理 `GrainInvocation`，调用一次 batch UDF，再返回逐 Grain
报告。
...
| [`_ready_fifo_by_callize_outputs`](../../rayorch/experimental/multigrain_v3_6/execution/worker.py#L292-L314) | raw return→列式合同 | 单输出/多输出括号语义是什么？ |
...
Worker 输入 `GrainInvocation` 只携带 binding；它无需知道 binding 原来属于哪个 ItemRef，因为 Engine 已
按 Call input 顺序投影完成。
...
| `GroupFailure` | `GrainFailureReport(suppress_siblings=True)` | 是 | Engine 建立同父 suppression barrier，不重试 |
...
## 6. `_input_columns()`：把 row-major GrainInvocations 转成 column-major UDF 参数
...
| `NestedGroupInput(bindings, offsets)` | 读取 leaves，再 `restore_nested_group()` 重建 nested list |
...
## 8. `_ready_fifo_by_callize_outputs()`：最容易写错的返回形状
...
## 9. 两种 Failure value：多输出如何保持逐 Grain 原子

normalize 后，Worker 先扫描所有输出列：只要第 i 行任意一列是 `RecordFailure` 或
`GroupFailure`，该 Grain 的全部输出都标为失败。
...
同一位置采用确定性 join：`GroupFailure > RecordFailure > normal`；cause 取 compiler output
layout 顺序中第一个最高优先级 sentinel。Worker 只设置内部行为位，不读取 parent/lineage，
barrier scope 由 Engine 在 driver 上解释。
...
`PortOutputReport.control`。业务 value 与 control 当前是同一个 bool，但通过独立字段越过协议边界，
...
Executor 把完整 WorkerDispatchResult 一次交给 Engine。page1 自身 `FAILED`；同 Call、同直接 PDF parent
且尚未提交的 page0/page2 success 会 `SUPPRESSED`。Worker 不认识 lineage，其他 PDF parent 的
报告仍正常提交。若这里只返回 `RecordFailure`，则 page0/page2 保持成功。
...
- 任一 output 的 Failure sentinel 是否封闭该 Grain 的全部 outputs，且 Failure 优先级稳定？
- expanded rows 是否只报告 binding，不创建语义 Entity？
- contract/UDF/RecordFailure/GroupFailure/infra failure 是否保持分层且只有 opaque/infra 进入恢复？
