...
- actor busy capacity；
- pending Ray ObjectRef→pending RPC 映射；
...
| 源码段 | 作用 | 核心不变量 |
| --- | --- | --- |
| [四个 driver-local DTO](../../rayorch/experimental/multigrain_v3_6/execution/executor.py#L30-L67) | counters、actor、microbatch、pending RPC | Ray 对象不泄漏到 Engine |
...
| [`run`](../../rayorch/experimental/multigrain_v3_6/execution/executor.py#L125-L257) | 完整 driver event loop | pending Ref 与 pending RPC 一一对应 |
...
pending ObjectRef 只保存为 dict key，pending RPC 是它返回后恢复完整上下文的唯一记录。generation 仍
在 GrainInvocation/DispatchState 中，不在 pending RPC 复制一份计数。
...
`_ready_fifo_by_callize_sources()` 把公开 `Sequence` 输入 eager materialize 为 tuples，并验证：
...
```text
1. active[index] 唯一拥有该 microbatch 的 Engine
2. 每个 pending ObjectRef 恰好映射到一个受 generation fence 保护的 pending RPC
3. busy actor 恰好对应一个 pending RPC，并在 finally 中释放
4. materialization 要求不存在 pending RPC，且 Engine 已完成
```
...
3. 从该 Engine reserve，最多 `pool.batch_size`；若只清理 barriered READY Grain，则本轮不发 RPC；
4. 对非空 live batch 逐 Grain 投影 `GrainInvocation`；
5. 取 compiler 生成的 output layouts；
6. 提交一次 actor RPC；
7. 标记 actor busy，并在 `pending_rpcs` 登记 `ObjectRef -> _PendingRpc`；
...
cleanup-only reserve 会发布 `SUPPRESSED` facts，并可能暴露一个本轮早先已经遍历过的 Call；
因此 `_dispatch_ready()` 返回 progress bit。若此时没有 pending RPC，event loop 直接开始下一轮，
不会把有效的本地状态推进误判成 deadlock。
...
```text
没有任何 pending RPC 引用其 index
且 engine.is_complete()
```
...
拿到一个 ref 后，先从 `pending_rpcs` pop `_PendingRpc`，再定位原 microbatch Engine。
...
`RecoveryAction`。若 suppression barrier 已命中 `DispatchBatch` 的一部分，先由 Engine 投影 live
subset，policy 只看 live Grain；barriered subset 不重试。非 ABORT 动作交给
Engine/DispatchState 执行，Executor
只更新真正 replay 的 Grain metrics。

### Infrastructure error

Ray `get()` 抛错说明 actor/transport 不可信。Engine 先封闭 barriered subset，再只为 live
subset 查询并执行 infrastructure retry budget；Executor 无论数据 subset 是否已全被 suppress，
仍替换不可信 actor。只有真正 live/requeue 的 Grain 计入 retry metrics。
...
`_PendingRpc` 持有 `_ActorSlot` 对象。基础设施失败时，Executor：
