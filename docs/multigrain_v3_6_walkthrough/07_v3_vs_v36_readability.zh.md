...
- value/block tables；
- receipt queue；
- Stage 的 ready、immediate-retry、deferred-recovery queues；
- pending RPCs；
- hard-limit counters；
...
| `DispatchState` | Grain phase/generation 与 ready/immediate-retry/deferred-recovery queues |
| `RecoveryPolicy` | 无状态恢复决策 |
| `Worker` | value-only UDF ABI |
| `Executor` | actor、pending RPC、capacity、多 microbatch 生命周期 |
...
queue、pending RPC、recovery budget、block ownership、batch timeout 与 timeline。两者性质不同。

---

## 12. `_fact_queue + advance()` 是否是多余抽象
...
因此问题是教程此前先使用 `_fact_queue/advance` 再定义它们，而不是机制本身没有必要。主教程已改为
