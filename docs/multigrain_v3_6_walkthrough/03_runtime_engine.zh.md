...
事实”，`_fact_queue` FIFO 保存“已经成立但尚待传播的事实身份”，`advance()` 则反复消费这些身份
...
| [`commit_reports`](../../rayorch/experimental/multigrain_v3_6/runtime/engine.py#L431-L491) | 完整 batch 预检、barrier discovery 与 mutation frontier | 必须精读 |
...
| `_suppression_barriers` | 哪些 Call+直接父级已被隔离？ | 只存单调 barrier，不复制 Grain 状态 |
| `_fact_queue` | 哪些新事实尚未传播？ | 临时通知，只存 Ref |
| `_entities_by_domain` | 某 Domain 已有哪些 Entity？ | canonical lineage 的枚举索引 |

`_fact_queue` 不保存 outcome、binding 或 children，因此它不是第二份真相。处理事件时总是拿 Ref 回到
...
只看 `MicrobatchEngine` 自己，它真正拥有的 FIFO 只有一个：`_fact_queue`。附近另外几种“等待”分别
属于不同 owner，不能都笼统叫 Engine queue：

| 结构 | 数据结构 | Owner | 保存什么 | 何时离开 |
| --- | --- | --- | --- | --- |
| `_fact_queue` | `deque[ItemRef \| ExpansionRef \| EntityRef]` | Engine | 已成立但尚未传播的事实身份 | `advance()` 消费 |
| `pending_grains` | `dict[GrainRef, PendingGrain]` | RuntimeState/Engine | 尚未由 Call input 代数决定的槽位 | Grain 变 READY 或直接 SEALED |
| `_ready_fifo_by_call` | `dict[CallRef, deque[GrainRef]]` | DispatchState | 按 Call 分区的首次可执行 Grain | 被 reserve 或 parent barrier suppress |
| `_immediate_retry_queue` | `deque[DispatchBatch]` | DispatchState | 需优先重试的精确 DispatchBatch | 被 reserve |
| `_deferred_recovery_queue` | `deque[DispatchBatch]` | DispatchState | 延后或二分后的 recovery batch | READY work 之后 reserve |
| `pending_rpcs` | `dict[ObjectRef, _PendingRpc]` | Executor | 已发出、尚未返回的 Ray RPC | `ray.wait/get` 完成 |

这张表同时解释了为什么不能把它们合成一个“任务队列”：

- `_fact_queue` 调度语义传播，不代表需要 Worker；
- `pending_grains` 是输入槽状态，不具有 FIFO 顺序；
- 三个 Dispatch queue 调度 Grain phase；
- Executor `pending_rpcs` 追踪物理 RPC，而不是语义 READY 状态。
...
前三类 publication 可能向 `_fact_queue` 追加 Ref；Grain phase 不进入 `_fact_queue`，它通过 DispatchState 的
...
`grain_invocation()` 把 semantic Item 变成纯物理 `RowBinding | NestedGroupInput | MissingInput`：
...
`ordered_items()`、`value_binding()`、`nested_group_rows()` 服务最终 materialization。`release_values()`
...
因此 `_fact_queue + advance()` 不是额外业务概念，而是 Engine 内部把“事实写入”和“事实传播”分开的
...
## 7. `commit_reports()`：为什么必须先看完整 WorkerDispatchResult

完整 WorkerDispatchResult 是全文件风险最高的入口，因为一个 Grain 可以同时产生：
...
而同一批后部的 `GroupFailure` 还必须压住前部同父 success。Executor 因此不再逐 report 提交；
源码先验证 DispatchBatch coverage，再按稳定的 `DispatchBatch.grains` 顺序发现全部 barrier，之后才进入 mutation frontier。

### 7.1 Phase 1：identity 与 barrier preflight

reports 必须与 `DispatchBatch.grains` 一一对应、无重复/缺失，且每个 generation 仍是
`IN_FLIGHT`。随后先收集所有 `GroupFailure` 的 `(CallRef, parent_anchor)`；同一个 WorkerDispatchResult 内
first-writer 由 `DispatchBatch.grains` 顺序决定，不由 report tuple 顺序决定。

### 7.2 Phase 2：逐 live output 预检并构造 publication intents
...
### 7.3 Phase 3：验证 aligned Expansion 的整体合同
...
### 7.5 Phase 4：按优先级和依赖顺序发布

每个 report 只走一条路径：显式 `RecordFailure/GroupFailure -> FAILED`；命中已有或本批 barrier
的 success `-> SUPPRESSED`；其他 success 才消费 prepared intents。被 suppress 的 expanded
payload 不创建 child Entity 或成功 Expansion。
...
## 8. 显式失败、suppression barrier 与恢复边界

最终业务失败与可恢复 dispatch 失败不是一回事。

### 最终失败提交

`_apply_failure()` 验证当前 attempt 后：
...
`GroupFailure` 仍把明确返回它的 Grain 发布为 `FAILED`，同时在 Engine 内建立
`(CallRef, immediate parent EntityRef)` barrier。同 scope 的 READY、后到输入和晚到 success
分别在 reserve/admission/commit 三个入口变为 `SUPPRESSED`；其他 parent/Call 不受直接影响，
已经提交的事实不回滚。
...
barrier 建立后若较早发出的 pending RPC 才发生 opaque/infra failure，Engine 先把其 DispatchBatch 分成 barriered
与 live subset：前者终态化但不消耗预算，后者才沿原策略 retry。基础设施失败仍由 Executor
替换 actor；actor 健康与数据隔离正交。
...
三个 gateway 是 `_fact_queue.append(...)` 的唯一位置，因此任何下游激活都来自已经写入 canonical
table 的事实。
...
| `READY` | DispatchState 创建 READY Grain 并入 ready queue |
...
value 已经是 `NestedGroupBinding` 时，Engine 使用 `NestedGroupLayout.nest()` 拼接子 CSR layout，并把所有
