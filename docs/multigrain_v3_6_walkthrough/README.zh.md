...
每篇尽量遵守同一解释顺序，避免再次出现“先使用 `_fact_queue`，后解释 `_fact_queue`”的问题：
...
Worker 篇先讲 BlockStore/GrainInvocation/layout，之后才进入执行流程。
...
| Worker | `GrainInvocation`、layout、批量 Python 值 | Program、Entity lineage、调度策略 |
| Executor | actor capacity、pending RPC、各 microbatch Engine | Filter/Reduce 的状态组合细节 |
...
- Executor 从 Engine 取得 `GrainInvocation`，不会自己拼 Item 或 lineage。
