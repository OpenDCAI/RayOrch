...
`batching_policy` 只是物理 packing 策略，不是语义作用域：

| 值 | 一个 `DispatchBatch` 可以包含 | 取舍 |
| --- | --- | --- |
| `"any_parent"` | 同一 Call 下任意直接父级的 READY Grain | 默认值，最大化聚批机会 |
| `"single_parent"` | 共享一个 `parent_anchor` 的 READY Grain | 局部性更强，但 batch 可能更小 |

两种取值都不改变 lineage、Reduce 语义或 `GroupFailure` 的同父 suppression scope。Executor
内部会把策略 lower 成更窄的 `pack_by_parent` 布尔值；该实现开关不属于 authoring API。
