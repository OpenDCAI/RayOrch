...
5. [`05：Worker ABI`](multigrain_v3_6_walkthrough/05_worker_abi.md)：`GrainInvocation` 怎样还原为 UDF
...
| [`静态语义编译边界`](multigrain_v3_6_compiler_boundary.md) | fixed compiler pipeline 与优化边界 | 为什么需要编译器；什么能优化、什么不能？ |
...
| [`双层数据失败与同父隔离实现`](todos/23-v36-parent-scoped-bad-item-isolation.md) | 已实现、待提交 | 记录 `RecordFailure` 精确失败、`GroupFailure` 同父隔离、batch commit、复杂度合同和回归证据 |
