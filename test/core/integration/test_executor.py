...
class IdentityPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.identity = ro.RayModule(PersistentIdentity).ray_options(
            batch_size=3,
            replicas=1,
            num_cpus=0,
        )
...
class KeywordPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.call = ro.RayModule(KeywordOnlyAdd).ray_options(
            batch_size=4,
            num_cpus=0,
        )
...
class CrashPipeline(ro.Pipeline):
    def __init__(self, marker: str) -> None:
        self.call = (
            ro.RayModule(CrashOnce)
            .pre_init(marker)
            .ray_options(
                batch_size=4,
                replicas=1,
...
class BadLeafPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.render = ro.RayModule(Render).ray_options(batch_size=8, num_cpus=0)
        self.transform = ro.RayModule(FailOneLeaf).ray_options(
            batch_size=16,
            num_cpus=0,
        )
        self.assemble = ro.RayModule(Assemble).ray_options(
            batch_size=8,
            num_cpus=0,
        )

    def forward(self, parents):
        leaves = ro.F.expand(self.render(parents))
        values = self.transform(leaves)
        return self.assemble(parents, ro.F.reduce(values))
...
class FailOneLeafGroup:
    def run(self, leaves):
        return [
            ro.GroupFailure(f"bad parent for {leaf}")
            if leaf == (1, 1)
            else leaf
            for leaf in leaves
        ]
...
class BadLeafGroupPipeline(ro.Pipeline):
    def __init__(self, *, transform_batch_size: int = 16) -> None:
        self.render = ro.RayModule(Render).ray_options(batch_size=8, num_cpus=0)
        self.transform = ro.RayModule(FailOneLeafGroup).ray_options(
    ...
    def run(self, leaves):
        ...
        for leaf in leaves:
            if leaf == (0, 0):
                ...
                results.append(ro.GroupFailure("bad parent"))
            else:
                results.append(leaf)
        return results


class LateSiblingPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.render = ro.RayModule(Render).ray_options(batch_size=1, num_cpus=0)
        self.transform = ro.RayModule(SlowSiblingGroup).ray_options(
            batch_size=1,
            replicas=2,
            num_cpus=0,
        )
    ...
    def forward(self, parents):
        return self.transform(ro.F.expand(self.render(parents)))
...
class MultiOutputPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.call = ro.RayModule(MultiOutputFail, num_outputs=2).ray_options(
            batch_size=8,
            num_cpus=0,
        )
...
class RetryPipeline(ro.Pipeline):
    def __init__(self, recovery: ro.RecoveryPolicy) -> None:
        self.call = ro.RayModule(FailFirstDispatch).ray_options(
            batch_size=2,
            replicas=1,
            recovery=recovery,
            num_cpus=0,
...
class IsolationPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.call = ro.RayModule(PoisonValue).ray_options(
            batch_size=4,
            recovery=ro.RecoveryPolicy.isolate_tail(),
            num_cpus=0,
        )
...
class AbortPipeline(ro.Pipeline):
    def __init__(self) -> None:
        self.call = ro.RayModule(AlwaysUdfError).ray_options(
            batch_size=4,
            num_cpus=0,
        )
...
def test_default_udf_policy_aborts_with_complete_dispatch_context():
    ...
    with pytest.raises(ro.ExecutionError) as captured:
...
class ContractPipeline(ro.Pipeline):
    def __init__(self, marker: str) -> None:
        self.call = (
            ro.RayModule(WrongCardinality)
            .pre_init(marker)
            .ray_options(
                batch_size=4,
                recovery=ro.RecoveryPolicy.isolate_tail(),
...
class ExhaustedInfrastructurePipeline(ro.Pipeline):
    def __init__(self, marker: str) -> None:
        self.call = (
            ro.RayModule(AlwaysCrash)
            .pre_init(marker)
            .ray_options(
                batch_size=4,
                recovery=ro.RecoveryPolicy.abort(infra_retries=1),
