...
PACKAGE = Path(__file__).parents[3] / "rayorch"
PACKAGE_NAME = ("rayorch",)


def _local_imports(path: Path) -> tuple[tuple[str, ...], ...]:
    """Resolve local absolute and relative imports below ``rayorch``."""
...
def test_package_dependency_boundaries() -> None:
    ...
    upper_layers = {"api", "functional", "_program", "_runtime", "_execution"}
    ...
    for path in PACKAGE.rglob("*.py"):
        ...
        for target in _local_imports(path):
            ...
            elif source_layer == "_program" and target_layer in {
                "api",
                "functional",
                "_runtime",
                "_execution",
            ...
            elif source_layer == "_runtime":
                if target_layer in {"api", "functional", "_execution"}:
                    reason = "runtime imports authoring or physical execution"
                elif target_layer == "_program" and target[:2] != ("_program", "plan"):
                    reason = "runtime may consume RuntimePlan, not compiler internals"
            elif relative.as_posix() == "_execution/worker.py" and target_layer not in {
                "_model",
                "_protocol",
                "failures",
                "result",
...
def test_program_phase_exports_are_deliberate() -> None:
    ...
    from rayorch._program import compiler, lowering, verify
