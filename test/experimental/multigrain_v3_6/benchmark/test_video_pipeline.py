"""Video v3.6 graph topology, UDF reuse and compiler parity tests."""
...
from rayorch.experimental.multigrain_v3_6.benchmark.video import (
    VideoCaptionV36Pipeline,
    VideoMultimodalV36Pipeline,
    VideoV36Pipeline,
)
from rayorch.experimental.multigrain_v3_6.benchmark.video import audit
...
def test_multimodal_video_has_two_sibling_relations_and_root_merge(optimize):
    compiled = VideoMultimodalV36Pipeline(
...
def test_paired_trial_materializes_v3_and_requires_exact_output(monkeypatch):
    ...
    monkeypatch.setattr(paired, "run_v36", fake_v36)
    ...
    assert calls == ["v36", "v3", "v3.get"]
    assert result["v3_rpc_count"] == result["v36_rpc_count"] == 3
...
def test_paired_trial_reports_first_business_output_difference(monkeypatch):
    ...
    new = _fake_v36_result([{"frames": 2}], rpc_count=1, grains=1)
    monkeypatch.setattr(paired, "run_v3", lambda *args, **kwargs: old)
    monkeypatch.setattr(paired, "run_v36", lambda *args, **kwargs: new)
...
def test_caption_model_pair_allows_bounded_text_drift_but_exact_structure(
    ...
    new = _fake_v36_result(new_outputs, rpc_count=3, grains=1)
    monkeypatch.setattr(model_paired, "run_caption_v3", lambda *a, **k: old)
    monkeypatch.setattr(model_paired, "run_caption_v36", lambda *a, **k: new)
...
def test_multimodal_model_pair_requires_exact_root_merge(monkeypatch):
    ...
    new = _fake_v36_result(list(outputs), rpc_count=7, grains=1)
    monkeypatch.setattr(model_paired, "run_multimodal_v3", lambda *a, **k: old)
    monkeypatch.setattr(model_paired, "run_multimodal_v36", lambda *a, **k: new)
    ...
    assert result["correctness"]["outputs_exact"]
    assert result["v3_rpc_count"] == result["v36_rpc_count"] == 7
