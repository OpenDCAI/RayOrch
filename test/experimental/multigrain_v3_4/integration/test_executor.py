...
from rayorch.experimental.multigrain_v3_4.benchmark.dummy import DummyPipeline
...
def test_actor_startup_failure_is_reported_before_run():
    """初始 UDF 构造失败必须在 Executor 构造阶段同步暴露。"""

    import ray

    with pytest.raises(ray.exceptions.RayActorError):
        Executor(StartupFailurePipeline())
...
def test_reusing_executor_does_not_accumulate_metrics():
...
def test_structural_relations_do_not_create_actors():
    """只有四个 RayModule Call 拥有 actor，所有 F.* 关系均为纯结构。"""
