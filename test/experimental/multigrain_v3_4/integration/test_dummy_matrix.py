...
from rayorch.experimental.multigrain_v3_4.benchmark.dummy_matrix import (
    NestedDummyPipeline,
    run_batching_matrix,
)
...
def test_ray_1000_parent_batch_cap_matrix_has_exact_parity_and_fewer_rpcs():
    records = run_batching_matrix(parents=1_000, rounds=2)
...
def test_nested_dummy_preserves_empty_intermediate_groups_and_order():
    ...
    assert result.arenas[0].is_complete()
    assert len(result.arenas[0].program.domains) == 3
