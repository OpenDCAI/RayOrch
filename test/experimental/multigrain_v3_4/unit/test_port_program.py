"""Port/Domain behavior matrix for the v3.4 static Program."""
...
def test_expand_rejects_non_call_output_instead_of_silently_losing_rows():
    class SourceExpand(mg.Pipeline):
        def forward(self, groups):
            return mg.functional.expand(groups)

    with pytest.raises(mg.CompileError, match="outputs of one producer Call"):
        SourceExpand().compile()


def test_group_valued_port_cannot_be_a_scalar_filter_mask():
    class GroupMask(mg.Pipeline):
        def __init__(self) -> None:
            self.render = mg.RayModule(U)

        def forward(self, rows):
            children = mg.functional.expand(self.render(rows))
            grouped = mg.functional.reduce(children)
            return mg.functional.filter(rows, grouped)

    with pytest.raises(mg.CompileError, match="group-valued Port"):
        GroupMask().compile()
