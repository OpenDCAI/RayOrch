...
from rayorch.experimental.multigrain_v3_3.program import ExpandOrigin
...
class DeepPipeline(mg.Pipeline):
    ...
    def __init__(self, depth: int, width: int) -> None:
        self.fanouts = [
            mg.RayModule(Fanout).pre_init(width).ray_options(batch_size=64)
            for _ in range(depth)
        ]
        self.sink = mg.RayModule(Pair).ray_options(batch_size=8)
...
def test_twenty_unary_levels_preserve_shape_depth_without_identity_shortcut():
    result = LocalExecutor(DeepPipeline(depth=20, width=1)).run([7])
    root, group = result.outputs[0]

    assert root == 7
    assert _nested_depth(group) == 20
    assert _nested_leaf_count(group) == 1
    deepest_domain = max(result.arena.program.domains)
    deepest = result.arena.entities(deepest_domain)
    assert len(deepest) == 1
    assert len(result.arena.entity_coordinate(deepest[0])) == 21


def test_five_binary_levels_preserve_all_32_ordered_leaves():
    result = LocalExecutor(DeepPipeline(depth=5, width=2)).run([3])
...
class AllFilteredPipeline(mg.Pipeline):
    ...
    def __init__(self) -> None:
        self.fanout = mg.RayModule(Fanout).pre_init(4)
        self.false = mg.RayModule(lambda values: [False for _ in values])
        self.left = mg.RayModule(Echo)
        self.right = mg.RayModule(Echo)
...
def test_all_filtered_is_present_empty_group_for_multiple_consumers():
    result = LocalExecutor(AllFilteredPipeline()).run([1, 2])

    assert result.outputs == ([[], []], [[], []])
    outcomes = [
        result.arena.item_outcome(item)
        for port in result.arena.program.output_tree
        for item in result.arena.ordered_items(port)
    ]
