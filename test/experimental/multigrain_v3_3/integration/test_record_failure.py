...
from rayorch.experimental.multigrain_v3_3.model import ItemOutcome
from rayorch.experimental.multigrain_v3_3.protocol import RecordFailure
...
class BadLeafPipeline(mg.Pipeline):
    ...
    def __init__(self) -> None:
        self.render = mg.RayModule(Render).ray_options(batch_size=8)
        self.transform = mg.RayModule(FailOneLeaf).ray_options(batch_size=16)
        self.assemble = mg.RayModule(Assemble).ray_options(batch_size=8)
...
def test_bad_leaf_suppresses_only_its_parent_inside_one_batch():
    result = LocalExecutor(BadLeafPipeline()).run([0, 1, 2])
    ...
    transform_metrics = result.calls[next(
        call for call, spec in result.arena.program.calls.items()
...
def test_record_failure_makes_all_outputs_of_one_grain_failed():
    left, right = LocalExecutor(MultiOutputPipeline()).run([1, 2, 3]).outputs
